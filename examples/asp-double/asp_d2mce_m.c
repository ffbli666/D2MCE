...
int main(int argc, char *argv[])
{
	...
	data = d2mce_malloc("data", sizeof(double)* n * n);
	tmp = d2mce_malloc("tmp", sizeof(double)*n);

	_data = malloc(sizeof(double)*n*n);
	//read data
	if (node_id == 0) {
...
*/
	d2mce_mstore(data,  node_id * block_size * n * sizeof(double), block_size * n * sizeof(double));
